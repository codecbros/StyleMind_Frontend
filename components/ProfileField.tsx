...
export function ProfileField({ label, value }: ProfileFieldProps) {
  const formattedValue = label === 'Fecha de Nacimiento' && value ? format(parseISO(value), 'dd/MM/yyyy') : value
  ...
  return (
    <div className='flex flex-col gap-1.5'>
      ...
      {label === 'Tono de piel' ? (
        value ? (
          <span className='w-8 h-8 rounded-full' style={{ backgroundColor: value }}></span>
        ) : (
